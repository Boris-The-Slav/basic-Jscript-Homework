...
function calcAge(birth) {
  ...
  if (!Number.isNaN(birthYear)) {
    if (currYear <= birthYear) {
      ...
      ageResult.innerText = `You were born in ${birthYear} and you are ${
        currYear - birthYear
      } years old`;
      birthYearInput.value = "";
    }
...
function convertYears(age) {
  ...
  if (!Number.isNaN(parsedAge)) {
    if (dogYears) {
      dogAgeConverted.innerText = `Your Dog is ${parsedAge} Human Years old and ${
        parsedAge * 7
      } Dog Years old.`;
      dogAge.value = "";
    } else {
      humanAge = +humanAge.toFixed(1);
      dogAgeConverted.innerText = `Your Dog is ${parsedAge} Dog Years old and ${
        parsedAge / 7
      } Human Years old.`;
      dogAge.value = "";
    }
